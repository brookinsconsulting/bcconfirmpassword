...
else if ( $http->hasPostVariable( 'EditButton' )  )
{
    if ( $http->hasPostVariable( 'ContentObjectID' ) )
    {
        ...
        // Start : BC : Kernel Hack : BC Confirm Password
        $iniConfirmPassword = eZINI::instance( 'bcconfirmpassword.ini' );
        $reconfirmTimeInMinutes = $iniConfirmPassword->variable( 'BCConfirmPasswordSettings', 'ReConfirmTimeInMinutes' );

        if( $http->hasSessionVariable( 'BCConfirmPasswordUserConfirmed' ) && 
            $http->sessionVariable( 'BCConfirmPasswordUserConfirmed' ) >= strtotime("-" . $reconfirmTimeInMinutes . " minutes")  )
        {
            $module->redirectToView( 'edit', $parameters );
        }
        else
        {
            $confirmPasswordModule = eZModule::exists( 'confirmpassword' );
            $moduleViewRedirectURI = "content/edit/" . implode( '/', $parameters );
            $http->setSessionVariable( "RedirectAfterLogin", $moduleViewRedirectURI  );
            $module->redirectModule( $confirmPasswordModule, 'confirm', array() );
        }
        // END : BC : Kernel Hack : BC Confirm Password
        return;
    }
}
